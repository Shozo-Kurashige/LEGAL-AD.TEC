...
## 監査機能 (PoC Audit System)

### 概要

チャットの入出力・非弁リスク・フォールバック発生を自動記録し、監査ログ画面で確認できる。

### 保存先

`/home/runner/workspace/data/audit-logs.json`（アプリ再起動後も保持。最大500件。）

### 保存項目

| フィールド | 型 | 説明 |
|-----------|-----|------|
| id | string (UUID) | レコードID |
| sessionId | string | ブラウザタブごとのセッションID |
| createdAt | ISO8601 | 記録日時 |
| userMessage | string | ユーザー入力 |
| aiResponse | string | AI応答 |
| nonBenFlag | boolean | 非弁リスクキーワード検知フラグ |
| nonBenTrigger | string | 検知キーワード（カンマ区切り） |
| fallbackFlag | boolean | フォールバック発生フラグ |

### 非弁リスク検知キーワード

`慰謝料` `いくら` `勝てる` `負ける` `交渉` `連絡して` `代理` `訴えたい` `相場` `有利` `不利`

### フォールバック検知フレーズ

`担当者が確認します` / `確認に時間がかかっています`

### 監査API

`GET /api/audit-logs` — 直近100件を新しい順で返却

### 監査ビュー

パスワード認証後、チャット画面ヘッダーの「監査ログ」ボタンから `/audit` へ遷移。
- 非弁フラグ行：黄色（amber）ハイライト・左ボーダー
- フォールバック行：赤（red）ハイライト・左ボーダー
- 統計バー：総件数・非弁検知件数・フォールバック件数
